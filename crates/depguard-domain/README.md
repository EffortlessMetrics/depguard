...
`depguard-domain` is depguard’s internal pure policy engine. Given an in-memory model and resolved configuration, it returns findings, verdicts, and summary data with no I/O.
...
## How to use
- Prefer depending on the public `depguard` facade crate for external integrations.
- Build or inject parsed models and resolved settings in an adapter.
- Call domain use functions to obtain results when working inside the workspace.
...
## Related crates
- `depguard` for the public Rust facade
