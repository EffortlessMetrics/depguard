...
pub fn parse_baseline_json(text: &str) -> anyhow::Result<DepguardBaselineV1> {
    let baseline: DepguardBaselineV1 = serde_json::from_str(text).context(format!(
        "Failed to parse baseline file as JSON. \
         Ensure the baseline file contains valid JSON with the required schema. \
         Expected format: {{ \"schema\": \"...\", \"fingerprints\": [...], \"findings\": [...] }}. \
         Common issues: missing commas, unescaped quotes, or truncated content. \
         Input length: {} bytes",
        text.len()
    ))?;
    if baseline.schema != SCHEMA_BASELINE_V1 {
        anyhow::bail!(
            "Unsupported baseline schema version: '{}'. Expected '{}'. \
             This may indicate the baseline file was generated by a different version of depguard. \
             Regenerate the baseline file using the current version: depguard baseline > baseline.json",
            baseline.schema,
            SCHEMA_BASELINE_V1
        );
    }
