...
pub fn parse_report_json(text: &str) -> anyhow::Result<ReportVariant> {
    let value: serde_json::Value = serde_json::from_str(text).context(format!(
        "Failed to parse report file as JSON. \
         \n\nThe report file must contain valid JSON with a recognized schema. \
         ...
         \n\nInput length: {} bytes \
         \n\nTo regenerate a valid report, run: depguard check --report-path <path>",
        text.len()
    ))?;
    ...
    match schema.as_str() {
        SCHEMA_REPORT_V2 | SCHEMA_SENSOR_REPORT_V1 => {
            let report: DepguardReportV2 = serde_json::from_value(value).context(format!(
                "Failed to parse report with schema '{}' as a valid depguard v2 report. \
                 \n\nThe schema field indicates v2 format but the content structure is invalid. \
                 \n\nThis error may occur if: \
                 \n  - The report was generated by an incompatible version of depguard \
                 \n  - The report file has been manually modified \
                 \n  - Required fields are missing or have incorrect types \
                 \n\nTo resolve: \
                 \n  1. Check depguard version: depguard --version \
                 \n  2. Regenerate the report: depguard check --report-path <path> \
                 \n  3. Ensure the report file has not been corrupted or truncated \
                 \n\nSchema: {}",
                schema, schema
            ))?;
        ...
        SCHEMA_REPORT_V1 | "receipt.envelope.v1" => {
            let report: DepguardReportV1 = serde_json::from_value(value).context(format!(
                "Failed to parse report with schema '{}' as a valid depguard v1 report. \
                 \n\nThe schema field indicates v1 format but the content structure is invalid. \
                 \n\nThis error may occur if: \
                 \n  - The report was generated by an incompatible version of depguard \
                 \n  - The report file has been manually modified \
                 \n  - Required fields are missing or have incorrect types \
            ...
            if let Ok(report) = serde_json::from_value::<DepguardReportV2>(value.clone()) {
                ...
                anyhow::bail!(
                    "Unknown report schema: '{}'. \
                     \n\nThe report schema field does not match any supported version. \
                     \n\nSupported schemas: \
                     \n  - {} (legacy v1 report) \
                     \n  - {} (current v2 report) \
                     \n  - {} (universal sensor report v1) \
                     \n  - receipt.envelope.v1 (legacy envelope) \
                     \n\nTo resolve: \
                     \n  1. Ensure the report was generated by depguard (not manually created) \
                     \n  2. Regenerate the report: depguard check --report-path <path> \
                     \n  3. Check that the report file has not been corrupted \
                     \n\nCurrent schema: '{}'",
                    schema,
                    SCHEMA_REPORT_V1,
                    SCHEMA_REPORT_V2,
...
pub fn serialize_report(report: &ReportVariant) -> anyhow::Result<Vec<u8>> {
    match report {
        ReportVariant::V1(r) => serde_json::to_vec_pretty(r).context(
            "Failed to serialize v1 report to JSON. \
             This is an internal error - the report data structure may contain invalid values. \
             Please report this issue if it persists.",
        ),
        ReportVariant::V2(r) => serde_json::to_vec_pretty(r).context(
            "Failed to serialize v2 report to JSON. \
             This is an internal error - the report data structure may contain invalid values. \
             Please report this issue if it persists.",
        ),
    }
...
mod tests {
    ...
    fn parse_report_json_unknown_schema_errors() {
        ...
        assert!(
            err_str.contains("Unknown report schema"),
            "Error should contain 'Unknown report schema'. Got: {err_str}"
        );
    }
    ...
    fn parse_report_rejects_invalid_json() {
        ...
        let err = parse_report_json(text).expect_err("expected parse error");
        let err_str = err.to_string();
        assert!(
            err_str.contains("Failed to parse report file as JSON"),
    ...
    #[test]
    fn parse_report_includes_input_length_on_error() {
    ...
    #[test]
    fn parse_report_rejects_unknown_schema() {
        let text = r#"{
            "schema": "depguard.report.v99",
            "tool": { "name": "depguard", "version": "0.1.0" },
            "run": { "started_at": "1970-01-01T00:00:00Z" },
            "verdict": { "status": "pass", "counts": { "info": 0, "warn": 0, "error": 0, "suppressed": 0 } },
            "findings": [],
            "data": { "scope": "repo", "profile": "strict" }
        }"#;

        let err = parse_report_json(text).expect_err("expected schema error");
        let err_str = err.to_string();
        assert!(
            err_str.contains("Unknown report schema"),
            "Error message should mention unknown schema. Got: {err_str}"
        );
        assert!(
            err_str.contains("depguard.report.v99"),
            "Error message should include the invalid schema. Got: {err_str}"
        );
        assert!(
            err_str.contains("Supported schemas"),
            "Error message should list supported schemas. Got: {err_str}"
        );
    }

    #[test]
    fn parse_report_v2_provides_context_on_parse_failure() {
        // Valid schema field but invalid structure
        let text = r#"{
        ...
        let err_str = err.to_string();
        assert!(
            err_str.contains("Failed to parse report with schema")
                || err_str.contains("Failed to parse report file"),
            "Error message should mention parsing failure. Got: {err_str}"
        );
    }
}
